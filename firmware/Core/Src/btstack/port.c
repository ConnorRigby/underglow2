...
void hal_uart_dma_set_sleep(uint8_t sleep){
	...
	// RTS is on PB12 - manually set it during sleep
	GPIO_InitTypeDef RTS_InitStruct;
	RTS_InitStruct.Pin = HCI_RTS_Pin;
    RTS_InitStruct.Pull = GPIO_NOPULL;
    RTS_InitStruct.Alternate = GPIO_AF8_LPUART1;
	if (sleep){
		HAL_GPIO_WritePin(HCI_RTS_GPIO_Port, HCI_RTS_Pin, GPIO_PIN_SET);
	...
	HAL_GPIO_Init(HCI_RTS_GPIO_Port, &RTS_InitStruct);
...
void EXTI15_10_IRQHandler(void){
	__HAL_GPIO_EXTI_CLEAR_IT(HCI_CTS_Pin);
	if (cts_irq_handler){
		(*cts_irq_handler)();
	}
}
...
void hal_uart_dma_set_csr_irq_handler( void (*the_irq_handler)(void)){
	...
	GPIO_InitTypeDef CTS_InitStruct = {
		...
		.Alternate = GPIO_AF8_LPUART1,
	};

	if( the_irq_handler )  {
		/* Configure the EXTI11 interrupt (LPUART_CTS is on PB13) */
		HAL_NVIC_EnableIRQ( EXTI15_10_IRQn );
		CTS_InitStruct.Mode = GPIO_MODE_IT_RISING;
		CTS_InitStruct.Pull = GPIO_NOPULL;
		HAL_GPIO_Init( HCI_CTS_GPIO_Port, &CTS_InitStruct );
...
void hal_uart_dma_send_block(const uint8_t *data, uint16_t size){
    HAL_UART_Transmit_DMA( &hlpuart1, (uint8_t *) data, size);
}
...
void port_main(void){
    ...
    hci_set_chipset(btstack_chipset_da145xx_instance());
